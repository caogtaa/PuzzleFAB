...
namespace PuzzLangLib {
    ...
    class Mover {
        ...
        static internal Mover Create(int obj, int cellindex, int layer, Direction dir, RuleGroup group = null,
            bool rigid = false) {
    ...
    class LevelState {
        ...
        internal void SetCell(int cellindex, int layer, int value) {
            ...
            if (_level[cellindex, layer] != value) {
                ...
                // update map
                if (value == 0)
                    _objectmap[cellindex].Remove(_level[locator]);
                else
                    _objectmap[cellindex].Add(value);
        ...
        bool ClearLocation(int cellindex, int layer) {
            if (_level[cellindex, layer] != 0) {
                ...
                var mover = _movers.FirstOrDefault(m =>
                    m.CellIndex == cellindex && _gamedef.GetLayer(m.ObjectId) == layer);
                if (mover != null)
                ...
                for (var movex = 0; movex < _movers.Count; ++movex) {
                    ...
                    if (mover.ObjectId == obj && mover.CellIndex == cellindex) {
                        ...
                        if (_movers[movex].Direction != direction) {
                            Logger.WriteLine(2, "Mover now {0} to {1} at {2}", direction, _gamedef.ShowName(obj),
                                cellindex);
                            if (direction == Direction.Stationary)
