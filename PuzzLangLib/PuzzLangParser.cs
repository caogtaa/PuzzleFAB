...
namespace PuzzLangLib {
    ...
    partial class PuzzLangParser {
        ...
        string DefSetting(Cursor cursor, string name, string value) {
            if (!_manager.SettingsParser.Parse(name, value))
                ParseError(cursor, $"invalid setting '{name}' value '{value}'");
        ...
        string DefObject(Cursor cursor, string ident, string glyph, IList<string> justify, string scale,
            ...
            if (gridlines.Count == 0) {
                _manager.AddObject(ident, jpivot, (float)nscale.Value, 1, new int[] { clist.First() }, clist.Last(),
                    message);
            } else {
            ...
            if (glyph != null)
                _manager.AddObjectDef(glyph, new List<string> { ident }, LogicOperator.None);
            return null;
        }
        ...
        string DefRule(Cursor state, IList<SubRule> pattern, IList<SubRule> action, IList<CommandCall> commands) {
            if (action.Count == 0) {
                // TODO: 这里action不可能为null
                if (action == null)
                    ParseError(state, "rule missing action or command");
            } else {
                var ok = (pattern.Count == action.Count)
                         && Enumerable.Range(0, pattern.Count)
                             .All(x => pattern[x].Cells.Count == action[x].Cells.Count);
                if (!ok)
                    ParseError(state, "rule pattern and action size mismatch");
            }

            // rule directions parsed on first subrule; others left as future enhancement
            // 除了第一个subrule外，其他subrule的prefix和direction都会被忽略
            if (pattern.Skip(1).Any(p => p.Prefixes.Any() || p.Directions.Any()) ||
                action.Any(p => p.Prefixes.Any() || p.Directions.Any()))
                _manager.CompileWarn("subrule prefixes or directions ignored");

            // just use the prefix and directions from first pattern subrule
            _manager.AddRule(LineNumber, pattern.First().Prefixes, pattern.First().Directions, pattern, action,
                commands);
            return null;
        }
    ...
    internal class Tracer : ITracer {
        ...
        public void TraceInfo(string ruleName, Cursor cursor, string info) {
            if (Enabled)
                Out.WriteLine($"Info rule: {ruleName} info: {info}");
        }

        public void TraceRuleEnter(string ruleName, Cursor cursor) {
            if (Enabled)
                Out.WriteLine($"Enter rule: {ruleName}");
        }
