...
namespace PuzzLangLib {
    ...
    class RuleIterator {
        ...
        internal bool HasAction {
            get { return Rule.HasAction; }
        }

        internal SubRule Pattern {
            get { return Rule.Patterns[SubRuleIndex]; }
        }

        internal SubRule Action {
            get { return Rule.Actions[SubRuleIndex]; }
        }

        internal IList<RuleAtom> PatternAtoms {
            get { return Pattern.Cells[CellIndex]; }
        }

        internal IList<RuleAtom> ActionAtoms {
            get { return Action.Cells[CellIndex]; }
        }

        internal bool Done {
            get { return SubRuleIndex >= Rule.Patterns.Count; }
        }
    ...
    class RuleBuilder {
        ...
        static readonly Dictionary<Direction, IList<Direction>> _ruledirlookup =
            new Dictionary<Direction, IList<Direction>> {
        ...
        static readonly Dictionary<Direction, Direction[]> _expanddirlookup = new Dictionary<Direction, Direction[]> {
            ...
            { Direction.Vertical, new Direction[] { Direction.Up, Direction.Down } }, {
                ...
                Direction.Moving, new Direction[] {
        ...
        IList<AtomicRule> ExpandDirectionSingle(RuleIterator riter, Direction olddir) {
            ...
            foreach (var newdir in _expanddirlookup[olddir]) {
                var newrule = riter.Rule.Clone();
                newrule.Patterns[riter.SubRuleIndex].Cells[riter.CellIndex] =
                    ReplaceDirection(riter.PatternAtoms, olddir, newdir);
                if (riter.HasAction)
                    newrule.Actions[riter.SubRuleIndex].Cells[riter.CellIndex] =
                        ReplaceDirection(riter.ActionAtoms, olddir, newdir);
        ...
        IList<AtomicRule> ExpandSymbolMulti(AtomicRule oldrule, ObjectSymbol oldsym) {
            ...
            foreach (var newobj in oldsym.ObjectIds) {
                ...
                for (var riter = new RuleIterator { Rule = oldrule }; !riter.Done; riter.Step()) {
                    newrule.Patterns[riter.SubRuleIndex].Cells[riter.CellIndex] =
                        ReplaceObject(riter.PatternAtoms, oldsym, newobj);
                    newrule.Actions[riter.SubRuleIndex].Cells[riter.CellIndex] =
                        ReplaceObject(riter.ActionAtoms, oldsym, newobj);
                }
        ...
        void CompilePatternAction(RuleAtom patom, RuleAtom aatom) {
            if (patom.IsNegated) CompileActionOnly(aatom);
                ...
                if (!(GameDef.MoveDirections.Contains(newdir) || newdir == Direction.Stationary ||
                      newdir == Direction.RandomDir))
