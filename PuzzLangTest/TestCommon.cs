    ...
    public class TestCommon {
        ...
        protected void DoTestModel(Compiler compiled, string inputs, string tests, string moreinfo) {
            ...
            foreach (var test in ts) {
                var tss = test.Split('=');
                if (tss.Length == 1)
                    Assert.AreEqual(test, result, moreinfo);
                if (tss[0] == "MSG")
                    Assert.AreEqual(tss[1], model.CurrentMessage ?? "null", moreinfo);
                if (tss[0] == "STA")
                    Assert.AreEqual(tss[1], model.Status ?? "null", moreinfo);
            }
        ...
        protected void DoTestLocationValue(Compiler compiled, string inputs, string predicts, string moreinfo) {
            ...
            for (int i = 0; i < objs.Length; i++) {
                var result = modelResult ??
                             (location == null ? "end" : model.GetObjects(location.Value).OrderBy(v => v).Join(""));
