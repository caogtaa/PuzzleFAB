    ...
    class Program {
        ...
        static readonly Dictionary<string, Action<string>> _options
            = new Dictionary<string, Action<string>>(StringComparer.OrdinalIgnoreCase) {
                ...
                { "take", (a) => { _take = a.SafeIntParse() ?? 9999; } }, {
                    "timing", (a) => {
                        _timing = true;
                        _settings.Add(Pair.Create("statistics_logging", "true"));
                    }
                },
        ...
        static void RunJsonConvTest(string path) {
            ...
            using (var rdr = new StringReader(script)) {
                ...
                while (line != null) {
                    var key = lookup.Keys.FirstOrDefault(
                        k => line.Length > k.Length && k == line.Substring(0, k.Length));
                    if (key != null)
                        lookup[key](line.Substring(key.Length).Trim());
                    line = rdr.ReadLine();
                }
            }
        ...
        static void RunJsonTest(string title, string script, string inputs, string startlevel, string finalstate,
            ...
            if (compiler.Success) {
                if (_skip.Split(',').Contains(counter.ToString()))
                    Logger.WriteLine(">{0} Skipped!", counter);
                else {
                    Logger.Pop();
                    if (_timing)
